...
// 4-step visible timeline — "Request Created" is implicit (we're already on the page).
// Starts directly at Searching so the UI never looks "done before it starts".
const PHASES = [
  { key: 'searching',  icon: Radio,        label: 'Searching Nearby',        sub: 'Broadcasting to available workers' },
  { key: 'expanding',  icon: Radio,        label: 'Expanding Search Radius', sub: null },
  { key: 'reviewing',  icon: Users,        label: 'Workers Notified',        sub: null },
  { key: 'accepted',   icon: CheckCircle2, label: 'Worker Accepted',         sub: 'Connecting you now…' },
];

const PHASE_ORDER = ['created', 'searching', 'expanding', 'reviewing', 'accepted'];
...
export default function DispatchProgressPanel() {
  ...
  // Map dispatchPhase → visible phase index.
  // 'created' shows as 'searching' (step 0 of visible list) since it's the starting state.
  const visiblePhase   = dispatchPhase === 'created' ? 'searching' : dispatchPhase;
  const visiblePhaseIdx = PHASES.findIndex((p) => p.key === visiblePhase);
  // -1 means accepted (beyond the list) or unknown
  const activeIdx = visiblePhaseIdx === -1 ? PHASES.length : visiblePhaseIdx;
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-4 pb-5">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40 mt-0.5">
                {elapsed(localElapsed)} elapsed
                {dispatchStep > 0 && ` · Step ${dispatchStep}/${dispatchTotalSteps || 10}`}
              </p>
            </div>
          </div>
        ...
        {/* Phase timeline — 4 steps, no redundant "Request Created" */}
              ...
              <div key={phase.key} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <motion.div
                    ...
                    animate={isActive ? { borderColor: ['rgba(99,102,241,0.5)', 'rgba(167,139,250,0.9)', 'rgba(99,102,241,0.5)'] } : {}}
                  ...
                  {idx < PHASES.length - 1 && (
                    <div className="w-px flex-1 mt-0.5 mb-0.5" style={{ minHeight: 18, background: isDone ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.07)' }} />
                  )}
                </div>

                <div className="pb-3 min-w-0 flex-1">
                  <p className={`text-[13px] font-bold leading-tight ${isDone ? 'text-green-400' : isActive ? 'text-white' : 'text-white/25'}`}>
                    {phase.label}
                  </p>
        ...
        {dispatchPhase === 'reviewing' && dispatchWorkersFound > 0 && (
          ...
          >
            <div className="flex gap-0.5">
              {Array.from({ length: Math.min(dispatchWorkersFound, 5) }).map((_, i) => (
                <motion.div
                  key={i}
            ...
            <p className="text-[11px] font-semibold text-indigo-300">Waiting for first acceptance</p>
          </motion.div>
        )}
      </div>
    </div>
